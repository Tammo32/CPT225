...
void printEnvStdout(Env env, NodeList* solution) {
    ...
    {
        if(solution->getNode(i)->getCol() 
                    < solution->getNode(i+1)->getCol()){
...
void testNode() {
    ...
    Node* node2 = new Node(4, 2, 3);


    std::cout << "Estimated_dist (should be 6): "
            << node1->getEstimatedDist2Goal(node2) << std::endl;
    delete node1;
    delete node2;
}
